...
// Build or stage the runtime autoloader
...
if (is_file($composer)) {
    ...
    // Copy clean runtime autoloader from source vendor directory
    if (is_dir($sourceDir . '/vendor/composer')) {
        @mkdir($stageDir . '/vendor/composer', 0755, true);
        copyDir($sourceDir . '/vendor/composer', $stageDir . '/vendor/composer', []);
        if (is_file($sourceDir . '/vendor/autoload.php')) {
            copy($sourceDir . '/vendor/autoload.php', $stageDir . '/vendor/autoload.php');
        }
    }
}
...
@mkdir($storageDir . '/backups', 0775, true);
@mkdir($storageDir . '/cache', 0775, true);
@mkdir($storageDir . '/logs', 0775, true);
@mkdir($storageDir . '/sessions', 0775, true);
@mkdir($storageDir . '/temp', 0775, true);
@file_put_contents($storageDir . '/backups/.gitkeep', '');
...
@file_put_contents($storageDir . '/temp/.gitkeep', '');
