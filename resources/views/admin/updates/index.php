...
<div class="wrap" style="max-width: 1040px; margin: 0 auto; padding-bottom: 40px;">
    ...
    <!-- In Progress Alert -->
    <?php if (($state['state'] ?? '') === 'RECOVERY_REQUIRED'): ?>
        <div class="notice notice-error"><strong>Recovery required:</strong> Maintenance stays active until the previous files and database are restored and verified. Keep the pre-update backup. <?php echo htmlspecialchars((string)($state['database_recovery_note'] ?? $state['rollback_error'] ?? $state['error'] ?? ''), ENT_QUOTES, 'UTF-8'); ?></div>
    ...
    <div class="row">
        ...
        <div class="col-md-7">
            ...
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    <?php elseif ($discovery['update_available']): ?>
                        <?php if (empty($discovery['sha256'])): ?>
                            <div class="notice notice-warning">This release does not provide a readable SHA-256 checksum. The archive structure will be checked, but its bytes cannot be compared with a published checksum.</div>
                        <?php endif; ?>
            ...
            <!-- Installed Extension Updates Card (shown only if updates are available for installed extensions) -->
            <?php if (!empty($extensionUpdates)): ?>
                <div class="card" style="border: 1px solid #c7d2fe; background: #f5f3ff; margin-bottom: 24px;">
                    <div class="card-header" style="background: #ede9fe; border-bottom: 1px solid #ddd6fe; display: flex; align-items: center; justify-content: space-between;">
                        <h5 style="color: #4c1d95; font-size: 15px; font-weight: 700; margin: 0; display: flex; align-items: center; gap: 8px;">
                            🧩 Installed Extension Updates
                        </h5>
                        <span class="badge" style="background: #7c3aed; color: #ffffff; font-size: 11px;">
                            <?php echo count($extensionUpdates); ?> Available
                        </span>
                    </div>
                    <div class="card-body" style="padding: 16px;">
                        <p style="font-size: 13px; color: #5b21b6; margin-bottom: 14px;">
                            New versions are available for installed extensions from the official distribution repository:
                        </p>

                        <?php foreach ($extensionUpdates as $ext): ?>
                            <div style="background: #ffffff; border: 1px solid #e9d5ff; border-radius: 6px; padding: 14px; margin-bottom: 12px;">
                                <div style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 8px;">
                                    <div>
                                        <strong style="font-size: 14px; color: #1e1b4b;"><?php echo htmlspecialchars((string)$ext['name']); ?></strong>
                                        <span class="badge badge-secondary" style="font-size: 11px; margin-left: 6px; text-transform: uppercase;">
                                            <?php echo htmlspecialchars((string)$ext['type']); ?>
                                        </span>
                                    </div>
                                    <div style="font-size: 12.5px;">
                                        <span class="text-muted">v<?php echo htmlspecialchars((string)$ext['installed_version']); ?></span>
                                        &rarr;
                                        <span class="badge badge-success" style="font-weight: 600;">v<?php echo htmlspecialchars((string)$ext['available_version']); ?></span>
                                    </div>
                                </div>

                                <?php if (!empty($ext['release_notes'])): ?>
                                    <details style="font-size: 12px; color: #64748b; margin-bottom: 10px;">
                                        <summary style="cursor: pointer; color: #6d28d9;">View release highlights</summary>
                                        <div style="margin-top: 6px; white-space: pre-wrap; font-family: monospace; font-size: 11px; background: #faf5ff; padding: 8px; border-radius: 4px;"><?php echo htmlspecialchars((string)$ext['release_notes']); ?></div>
                                    </details>
                                <?php endif; ?>

                                <form method="POST" action="<?php echo $base; ?>/admin/updates/apply-extension" onsubmit="return confirm('Update <?php echo htmlspecialchars((string)$ext['name']); ?> to version <?php echo htmlspecialchars((string)$ext['available_version']); ?>?');">
                                    <input type="hidden" name="_token" value="<?php echo htmlspecialchars($csrfToken); ?>">
                                    <input type="hidden" name="type" value="<?php echo htmlspecialchars((string)$ext['type']); ?>">
                                    <input type="hidden" name="id" value="<?php echo htmlspecialchars((string)$ext['id']); ?>">
                                    <input type="hidden" name="version" value="<?php echo htmlspecialchars((string)$ext['available_version']); ?>">
                                    <input type="hidden" name="download_url" value="<?php echo htmlspecialchars((string)$ext['download_url']); ?>">
                                    <input type="hidden" name="sha256" value="<?php echo htmlspecialchars((string)$ext['sha256']); ?>">
                                    <button type="submit" class="btn btn-primary" style="background: #7c3aed; border-color: #6d28d9; padding: 6px 14px; font-size: 12.5px; font-weight: 600;">
                                        ⚡ Update <?php echo ucfirst((string)$ext['type']); ?> Now
                                    </button>
                                </form>
                            </div>
                        <?php endforeach; ?>
