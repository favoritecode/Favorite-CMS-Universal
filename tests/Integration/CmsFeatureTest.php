...
class CmsFeatureTest extends TestCase
{
    ...
    public function testThemeManager(): void
    {
        ...
        $this->assertIsString($manager->getActiveTheme());
    }

    public function testPluginManager(): void
    {
        $testPluginDir = APP_ROOT . '/plugins/test-feature-plugin';
        if (!is_dir($testPluginDir)) {
            @mkdir($testPluginDir, 0775, true);
        }
        file_put_contents($testPluginDir . '/plugin.json', (string)json_encode([
            'id'      => 'test-feature-plugin',
            'name'    => 'Test Feature Plugin',
            'version' => '1.0.0',
            'entry'   => 'plugin.php',
        ]));
        file_put_contents($testPluginDir . '/plugin.php', "<?php if (!defined('TEST_FEATURE_PLUGIN_LOADED')) define('TEST_FEATURE_PLUGIN_LOADED', true);");

        try {
            $manager = new PluginManager(static::$app);
            $plugins = $manager->getInstalledPlugins();

            $this->assertArrayHasKey('test-feature-plugin', $plugins);

            // Test activation
            $manager->activatePlugin('test-feature-plugin');
            $this->assertContains('test-feature-plugin', $manager->getActivePlugins());

            // Test boot
            $manager->bootActivePlugins();
            $this->assertTrue(defined('TEST_FEATURE_PLUGIN_LOADED'));

            // Test deactivation
            $manager->deactivatePlugin('test-feature-plugin');
            $this->assertNotContains('test-feature-plugin', $manager->getActivePlugins());
        } finally {
            @unlink($testPluginDir . '/plugin.json');
            @unlink($testPluginDir . '/plugin.php');
            @rmdir($testPluginDir);
        }
    }
