...
class PluginReadinessTest extends TestCase
{
    ...
    public function testPluginLifecycleEventTriggers(): void
    {
        ...
        $testPluginId = 'test-lifecycle-plugin';
        $testPluginDir = APP_ROOT . '/plugins/' . $testPluginId;
        if (!is_dir($testPluginDir)) {
            mkdir($testPluginDir, 0777, true);
        }
        file_put_contents($testPluginDir . '/plugin.json', json_encode([
            'id' => $testPluginId,
            'name' => 'Lifecycle Test Plugin',
            'version' => '1.0.0',
        ]));
        file_put_contents($testPluginDir . '/plugin.php', '<?php // test plugin');
