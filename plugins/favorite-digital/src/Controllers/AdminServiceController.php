...
class AdminServiceController
{
    ...
    /**
     * Dispatcher for admin service requests.
     */
    public function handle(Request $request): Response|string
    {
        ...
        if ($currentUser && method_exists($currentUser, 'can') && !$currentUser->can('manage_options')) {
            return Response::make('<h1>403 Access Denied</h1><p>You do not have permission to manage services.</p>', 403);
    ...
    protected function handlePost(Request $request): Response
    {
        if (!$this->validateCsrf($request)) {
            ...
            return Response::redirect('/admin/page/favorite-digital-services');
        }
        ...
        return match ($action) {
            ...
            default   => Response::redirect('/admin/page/favorite-digital-services'),
        };
    }
    ...
    public function index(Request $request): string
    {
        ...
        return $this->renderView('services/index', [
            'services'      => $services,
    ...
    public function create(Request $request): string
    {
        ...
        return $this->renderView('services/create', [
            'old'        => $old,
            'csrfToken'  => $this->getCsrfToken(),
            'flashError' => $_SESSION['flash_error'] ?? null,
        ]);
    ...
    public function store(Request $request): Response
    {
        ...
        $serviceScope = $request->post('service_scope', $request->post('scope_description', ''));
        $serviceInput = [
            'delivery_time_days'  => ($deliveryDays !== '' && $deliveryDays !== null) ? (int)$deliveryDays : 1,
            'service_scope'       => (string)$serviceScope,
        ...
        try {
            ...
            $_SESSION['flash_error'] = 'Failed to create service: ' . $e->getMessage();
            $_SESSION['old_input'] = array_merge($productInput, $serviceInput);
            return Response::redirect('/admin/page/favorite-digital-services?action=create');
        }
    }
    ...
    public function edit(Request $request, int $id): Response|string
    {
        ...
        if (!$product || $product->product_type !== ProductType::SERVICE) {
            $_SESSION['flash_error'] = 'Service not found.';
            return Response::redirect('/admin/page/favorite-digital-services');
        }

        $details = $this->service->getRepository()->findServiceDetails($id);
        $old = $_SESSION['old_input'] ?? null;
        unset($_SESSION['old_input']);

        return $this->renderView('services/edit', [
            'product'      => $product,
            'details'      => $details,
            'old'          => $old,
            'csrfToken'    => $this->getCsrfToken(),
    ...
    public function update(Request $request, int $id): Response
    {
        ...
        try {
            ...
            $_SESSION['flash_error'] = 'Failed to update service: ' . $e->getMessage();
            $_SESSION['old_input'] = array_merge($productInput, $serviceInput);
            return Response::redirect('/admin/page/favorite-digital-services?action=edit&id=' . $id);
        }
    }
    ...
    public function view(Request $request, int $id): Response|string
    {
        ...
        return $this->renderView('services/view', [
            'product'      => $product,
            'details'      => $details,
            'csrfToken'    => $this->getCsrfToken(),
            'flashSuccess' => $_SESSION['flash_success'] ?? null,
    ...
    public function publish(Request $request, int $id): Response
    {
        try {
            ...
            $_SESSION['flash_success'] = "Service #{$id} published successfully.";
        } catch (Throwable $e) {
            $_SESSION['flash_error'] = 'Publish failed: ' . $e->getMessage();
        }

        $redirect = $request->post('redirect_to', '/admin/page/favorite-digital-services?action=view&id=' . $id);
    ...
    public function draft(Request $request, int $id): Response
    {
        try {
            ...
            $_SESSION['flash_success'] = "Service #{$id} switched to draft.";
        } catch (Throwable $e) {
    ...
    public function archive(Request $request, int $id): Response
    {
        try {
            ...
            $_SESSION['flash_success'] = "Service #{$id} archived successfully.";
        } catch (Throwable $e) {
