...
<div class="fd-admin-wrap" style="max-width: 1200px; margin: 0 auto; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif;">
    ...
    <div style="display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 16px; margin-bottom: 20px;">
        <div style="display: flex; gap: 8px; align-items: center;">
            <span style="font-size: 20px; font-weight: 600; color: #1e1e1e;">Services</span>
            <span style="background: #f0f0f1; color: #50575e; padding: 3px 8px; border-radius: 12px; font-size: 13px; font-weight: 500;">
                <?php echo (int)($counts['all'] ?? count($services)); ?> total
            </span>
        </div>
        <div>
            <a href="/admin/page/favorite-digital-services?action=create" style="display: inline-flex; align-items: center; gap: 6px; background: #2271b1; color: #fff; text-decoration: none; padding: 8px 16px; border-radius: 4px; font-weight: 500; font-size: 14px;">
    ...
    <div style="display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 12px; margin-bottom: 16px;">
        ...
        <div style="display: flex; gap: 8px; font-size: 13px;">
            <a href="/admin/page/favorite-digital-services" style="text-decoration: none; padding: 4px 10px; border-radius: 4px; <?php echo ($statusFilter === null) ? 'background: #2271b1; color: #fff; font-weight: 600;' : 'color: #2271b1; background: #f6f7f7;'; ?>">
                All (<?php echo (int)($counts['all'] ?? 0); ?>)
            </a>
            <a href="/admin/page/favorite-digital-services?status=published" style="text-decoration: none; padding: 4px 10px; border-radius: 4px; <?php echo ($statusFilter === 'published') ? 'background: #2271b1; color: #fff; font-weight: 600;' : 'color: #2271b1; background: #f6f7f7;'; ?>">
                Published (<?php echo (int)($counts['published'] ?? 0); ?>)
            </a>
            <a href="/admin/page/favorite-digital-services?status=draft" style="text-decoration: none; padding: 4px 10px; border-radius: 4px; <?php echo ($statusFilter === 'draft') ? 'background: #2271b1; color: #fff; font-weight: 600;' : 'color: #2271b1; background: #f6f7f7;'; ?>">
                Draft (<?php echo (int)($counts['draft'] ?? 0); ?>)
            </a>
            <a href="/admin/page/favorite-digital-services?status=archived" style="text-decoration: none; padding: 4px 10px; border-radius: 4px; <?php echo ($statusFilter === 'archived') ? 'background: #2271b1; color: #fff; font-weight: 600;' : 'color: #2271b1; background: #f6f7f7;'; ?>">
                Archived (<?php echo (int)($counts['archived'] ?? 0); ?>)
            </a>
        </div>

        <!-- Search Form -->
        <form method="GET" action="/admin/page/favorite-digital-services" style="display: flex; gap: 6px; margin: 0;">
            <?php if ($statusFilter): ?>
                <input type="hidden" name="status" value="<?php echo htmlspecialchars($statusFilter, ENT_QUOTES, 'UTF-8'); ?>">
            <?php endif; ?>
            <input type="text" name="q" value="<?php echo htmlspecialchars((string)($search ?? ''), ENT_QUOTES, 'UTF-8'); ?>" placeholder="Search services..." style="padding: 6px 12px; border: 1px solid #8c8f94; border-radius: 4px; font-size: 13px; width: 220px;">
            ...
            <?php if (!empty($search)): ?>
                <a href="/admin/page/favorite-digital-services<?php echo $statusFilter ? '?status=' . urlencode($statusFilter) : ''; ?>" style="padding: 6px 10px; color: #646970; text-decoration: none; font-size: 13px; align-self: center;">Reset</a>
            <?php endif; ?>
        </form>
    </div>
                        ...
                        <tr style="border-bottom: 1px solid #f0f0f1;">
                            ...
                            <td style="padding: 12px 14px; text-align: right;">
                                <div style="display: inline-flex; align-items: center; gap: 8px;">
                                    <a href="/admin/page/favorite-digital-services?action=view&id=<?php echo (int)$s->id; ?>" style="color: #2271b1; text-decoration: none; font-weight: 500;">View</a>
