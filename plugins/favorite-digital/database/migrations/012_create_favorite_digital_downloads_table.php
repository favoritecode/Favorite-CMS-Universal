<?php

declare(strict_types=1);

use FavoriteCMS\Core\Database;

/**
 * Favorite Digital — Migration 012: Downloads Table
 *
 * Secure download tokens, IP/User-Agent tracking, and download access history logs.
 */
class CreateFavoriteDigitalDownloadsTable
{
    protected Database $db;

    public function __construct(Database $db)
    {
        $this->db = $db;
    }

    public function up(): void
    {
        $isSqlite = $this->isSqlite();
        $engine = $isSqlite ? '' : ' ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci';
        $pkBigint = $isSqlite ? 'INTEGER PRIMARY KEY AUTOINCREMENT' : 'BIGINT AUTO_INCREMENT PRIMARY KEY';

        $this->db->execute("
            CREATE TABLE IF NOT EXISTS `favorite_digital_downloads` (
                `id`             {$pkBigint},
                `entitlement_id` BIGINT       NOT NULL,
                `product_id`     BIGINT       NOT NULL,
                `user_id`        BIGINT       NOT NULL,
                `download_token` VARCHAR(64)  NOT NULL UNIQUE,
                `ip_address`     VARCHAR(45)  NULL,
                `user_agent`     TEXT         NULL,
                `downloaded_at`  DATETIME     NULL,
                `download_count` INT          NOT NULL DEFAULT 0,
                `created_at`     TIMESTAMP    DEFAULT CURRENT_TIMESTAMP
            ){$engine};
        ");

        $this->createIndexIfNotExists('favorite_digital_downloads', 'idx_fd_dl_token', '`download_token`', true);
        $this->createIndexIfNotExists('favorite_digital_downloads', 'idx_fd_dl_ent', '`entitlement_id`');
        $this->createIndexIfNotExists('favorite_digital_downloads', 'idx_fd_dl_user', '`user_id`');
        $this->createIndexIfNotExists('favorite_digital_downloads', 'idx_fd_dl_prod', '`product_id`');
    }

    public function down(): void
    {
        $this->db->execute("DROP TABLE IF EXISTS `favorite_digital_downloads`");
    }

    protected function isSqlite(): bool
    {
        try {
            $driver = $this->db->getConnection()->getAttribute(\PDO::ATTR_DRIVER_NAME);
            return strtolower((string)$driver) === 'sqlite';
        } catch (\Throwable) {
            return false;
        }
    }

    protected function createIndexIfNotExists(string $table, string $indexName, string $columns, bool $unique = false): void
    {
        if ($this->isSqlite()) {
            $uniqueClause = $unique ? 'UNIQUE' : '';
            $this->db->execute("CREATE {$uniqueClause} INDEX IF NOT EXISTS `{$indexName}` ON `{$table}` ({$columns})");
            return;
        }

        try {
            $existing = $this->db->select("SHOW INDEX FROM `{$table}` WHERE Key_name = ?", [$indexName]);
            if (empty($existing)) {
                $uniqueClause = $unique ? 'UNIQUE' : '';
                $this->db->execute("ALTER TABLE `{$table}` ADD {$uniqueClause} INDEX `{$indexName}` ({$columns})");
            }
        } catch (\Throwable) {
            try {
                $uniqueClause = $unique ? 'UNIQUE' : '';
                $this->db->execute("ALTER TABLE `{$table}` ADD {$uniqueClause} INDEX `{$indexName}` ({$columns})");
            } catch (\Throwable) {
            }
        }
    }
}

