...
class Kernel
{
    ...
    public function handle(Request $request): Response
    {
        try {
            ...
            // PHP leaves form arrays empty for JSON as well as oversized forms.
            $postLimit = \FavoriteCMS\Services\UploadCapabilityService::parseByteString((string)ini_get('post_max_size'));
            $contentType = strtolower(trim(explode(';', (string)($_SERVER['CONTENT_TYPE'] ?? ''))[0]));
            if (
                strtoupper((string)($_SERVER['REQUEST_METHOD'] ?? 'GET')) === 'POST' &&
                in_array($contentType, ['application/x-www-form-urlencoded', 'multipart/form-data'], true) &&
                $postLimit > 0 && (int)($_SERVER['CONTENT_LENGTH'] ?? 0) > $postLimit &&
    ...
    protected function dispatchAdmin(Request $request, string $path, string $method): Response
    {
        ...
        $mutation = preg_match('#^/admin/(posts|pages|taxonomies|media|comments|users|menus|themes|widgets|customize|plugins|settings|seo|tools|updates)/(?:.*/)?(store|update|approve|reject|trash|restore|delete|bulk|quick-draft|unapprove|spam|create|add|location|activate|deactivate|upload|upload-ajax|reorder|move|duplicate|reset|save|delete-account|apply|rollback|cancel-upload|process|blogger|verify|test-email)$#', $path) === 1
            || in_array($path, ['/admin/users/status', '/admin/users/role', '/admin/users/verify', '/admin/settings/test-email', '/admin/settings/test-smtp', '/admin/settings/google-auth', '/admin/settings/google-disconnect', '/admin/settings/test-google'], true);
        ...
        if (str_starts_with($path, '/admin/settings')) {
            ...
            return match ($path) {
                ...
                '/admin/settings/google-auth'        => $ctrl->startGoogleAuth($request),
                '/admin/settings/google-callback'    => $ctrl->handleGoogleCallback($request),
                '/admin/settings/google-disconnect'  => $ctrl->disconnectGoogle($request),
                '/admin/settings/test-google'        => $ctrl->testGoogleConnection($request),
        ...
        if (str_starts_with($path, '/admin/updates')) {
            ...
            return match ($path) {
                ...
                '/admin/updates/apply-extension' => $ctrl->applyExtension($request),
