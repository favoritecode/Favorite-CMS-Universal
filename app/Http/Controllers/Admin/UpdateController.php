...
class UpdateController
{
    ...
    protected ExtensionReleaseDiscovery $extensionDiscovery;

    public function __construct(Application $app)
    {
        $this->app = $app;
        $this->updateManager = new UpdateManager($app);
        $this->extensionDiscovery = new ExtensionReleaseDiscovery($app);
    }
    ...
    public function index(Request $request): Response
    {
        ...
        $extensionUpdates = $this->extensionDiscovery->checkUpdates($forceCheck);
    ...
    public function applyExtension(Request $request): Response
    {
        $this->validateCsrf($request);

        $type = (string)$request->post('type', '');
        $id = (string)$request->post('id', '');
        $downloadUrl = (string)$request->post('download_url', '');
        $targetVersion = (string)$request->post('version', '');
        $expectedSha256 = (string)$request->post('sha256', '');

        if ($type !== 'plugin' && $type !== 'theme') {
            $_SESSION['_flash_error'] = "Invalid extension type requested.";
            return Response::redirect('/admin/updates');
        }

        if (empty($id) || empty($downloadUrl) || !filter_var($downloadUrl, FILTER_VALIDATE_URL)) {
            $_SESSION['_flash_error'] = "Invalid extension update parameters.";
            return Response::redirect('/admin/updates');
        }
        ...
        $tmpZip = $tempDir . '/ext_update_' . bin2hex(random_bytes(6)) . '.zip';

        try {
            // Download package
            $this->updateManager->getDiscovery()->downloadPackage($downloadUrl, $tmpZip);

            // Optional SHA-256 verification
            if (!empty($expectedSha256)) {
                $actualSha256 = strtolower(hash_file('sha256', $tmpZip));
                if (!hash_equals(strtolower($expectedSha256), $actualSha256)) {
                    throw new \RuntimeException("Checksum mismatch for downloaded extension package.");
                }
            }

            if ($type === 'plugin') {
                $pluginMgr = new \FavoriteCMS\Plugins\PluginManager($this->app);
                $pluginMgr->installFromZip(['tmp_name' => $tmpZip]);
            } else {
                $themeMgr = new \FavoriteCMS\Themes\ThemeManager($this->app);
                $themeMgr->installFromZip(['tmp_name' => $tmpZip]);
            }

            // Invalidate extension update cache
            $cacheFile = APP_ROOT . '/storage/cache/extension_updates.json';
            if (is_file($cacheFile)) {
                @unlink($cacheFile);
            }

            $label = ucfirst($type) . " '{$id}'";
            $verMsg = $targetVersion !== '' ? " to version {$targetVersion}" : "";
            $_SESSION['_flash_notice'] = "Successfully updated {$label}{$verMsg}!";

        } catch (Throwable $e) {
            $_SESSION['_flash_error'] = "Failed to update " . ucfirst($type) . " '{$id}': " . $e->getMessage();
        } finally {
            if (is_file($tmpZip)) {
                @unlink($tmpZip);
            }
