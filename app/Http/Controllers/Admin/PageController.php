...
class PageController
{
    ...
    public function update(Request $request): Response
    {
        ...
        try {
            $content = ContentRevision::clean($content, $page, 'page', $actor, (string)$request->post('_content_revision', ''));
    ...
    public function preview(Request $request): Response
    {
        ...
        $original = $originalId > 0 ? Page::find($originalId) : null;
        if (!$actor || !$actor->canManagePages() || ($originalId > 0 && !$original)) {
            return Response::make('You do not have permission to preview this content.', 403);
        }
        try {
            $cleanedContent = ContentRevision::clean($content, $original, 'page', $actor, (string)$request->post('_content_revision', ''));
        } catch (\RuntimeException $error) {
