...
class PostController
{
    ...
    public function store(Request $request): Response
    {
        ...
        \FavoriteCMS\Services\DraftReceipt::saved($request, $currentUser);
    ...
    public function update(Request $request): Response
    {
        ...
        try {
            $content = ContentRevision::clean($content, $post, 'post', $actor, (string)$request->post('_content_revision', ''));
    ...
    public function preview(Request $request): Response
    {
        ...
        $originalId = (int)$request->post('id', 0);
        $original = $originalId > 0 ? Post::find($originalId) : null;
        if (!$actor || !$actor->canCreatePosts() || ($original && !$actor->canEditPost($original)) || ($originalId > 0 && !$original)) {
            return Response::make('You do not have permission to preview this content.', 403);
        }
        try {
            $cleanedContent = ContentRevision::clean($content, $original, 'post', $actor, (string)$request->post('_content_revision', ''));
        } catch (\RuntimeException $error) {
            return Response::make(htmlspecialchars($error->getMessage(), ENT_QUOTES, 'UTF-8'), 409);
        }

        $fakePost = new Post([
            '_preview_content_verified' => true,
            'id'                => 0,
            'title'             => $title ?: 'Preview',
            'slug'              => 'preview',
    ...
    public function quickDraft(Request $request): Response
    {
        ...
        if ($title !== '') {
            ...
            $authorId = (int)$currentUser->id;
            $content = ContentSanitizer::clean($content, $currentUser);

            $db = $this->app->make(Database::class);
            $draftId = $db->insert('posts', [
                'title'      => $title,
                'slug'       => $slug,
                'content'    => $content,
                'status'     => 'draft',
            ...
            ContentRevision::record(Post::find($draftId), 'post');
            $_SESSION['flash_success'] = 'Draft saved successfully.';
        }
