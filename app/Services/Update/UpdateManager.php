...
class UpdateManager
{
    ...
    {
        if (($this->getState()['state'] ?? '') === self::STATE_RECOVERY_REQUIRED && $this->maintenance->isActive()) {
            return true;
        }
    ...
    {
        if ($this->isUpdateInProgress()) {
            throw new RuntimeException('An update or recovery is already in progress. Complete recovery before trying another update.');
        }
        ...
        try {
            ...
            $db = $this->app->make(Database::class);
            $migrator = new Migrator($db);
            // Mark potential DDL changes before execution; this survives later health failures.
            $databaseMayHaveChanged = !$db->tableExists('cms_migrations');
            if (!$databaseMayHaveChanged) {
                foreach ($migrator->getMigrationFiles($this->appRoot . '/database/migrations') as $migrationFile) {
                    if (!$migrator->hasRun(basename($migrationFile, '.php'))) {
            ...
            if (!$this->maintenance->disable()) {
                throw new RuntimeException('Could not disable maintenance after update.');
            }
            ...
            $recoveryRequired = $databaseMayHaveChanged;
            if ($filesTouched && $coreBackupReady) {
            ...
            // Never reopen a mixed filesystem or potentially migrated database to visitors.
            if ($filesTouched && !$coreBackupReady) $recoveryRequired = true;
            if ($recoveryRequired) {
                $this->updateState(self::STATE_RECOVERY_REQUIRED, ['database_rollback_required' => $databaseMayHaveChanged]);
                if (!$this->maintenance->isActive()) {
                    $this->maintenance->enable(['phase' => self::STATE_RECOVERY_REQUIRED]);
                }
            } elseif ($maintenanceEnabled && !$this->maintenance->disable()) {
                $this->updateState(self::STATE_RECOVERY_REQUIRED, ['error' => 'Could not clear maintenance after file rollback.']);
            }
            $this->releaseLock();
    ...
    protected function restoreCoreFiles(string $backupSource): void
    {
        ...
        if (!empty($this->newlyAddedMigrations)) {
            foreach ($this->newlyAddedMigrations as $newMigration) {
                if (file_exists($newMigration)) {
                    if (!@unlink($newMigration)) throw new RuntimeException('Could not remove migration during rollback: ' . $newMigration);
                }
            }
        ...
        foreach ($coreFiles as $file) {
            ...
            if (is_file($src)) {
                ...
                if (!@unlink($dst)) throw new RuntimeException('Could not remove file during rollback: ' . $dst);
            }
        }
    }
        ...
        if (is_dir($srcMigrations)) {
            ...
            if ($files) {
                foreach ($files as $file) {
                    ...
                    $this->copyFileVerified($file, $targetFile);
                }
            }
        }
        ...
        if (is_dir($srcConfig)) {
            ...
            if ($configFiles) {
                foreach ($configFiles as $cFile) {
                    ...
                    if (!is_file($target)) {
                        $this->copyFileVerified($cFile, $target);
                    }
                }
            }
        }
    ...
    protected function copyFileVerified(string $source, string $destination): void
    {
        $parent = dirname($destination);
        if (!is_dir($parent) && !@mkdir($parent, 0775, true) && !is_dir($parent)) {
            throw new RuntimeException('Cannot create update directory: ' . $parent);
        }
        $temporary = $destination . '.update-' . bin2hex(random_bytes(8));
        try {
            $expected = @hash_file('sha256', $source);
            if ($expected === false || !@copy($source, $temporary) || @hash_file('sha256', $temporary) !== $expected) {
                throw new RuntimeException('Cannot copy complete update file: ' . $destination);
            }
            if (is_file($destination)) @chmod($temporary, fileperms($destination) & 0777);
            if (!@rename($temporary, $destination) || @hash_file('sha256', $destination) !== $expected) {
                throw new RuntimeException('Cannot replace update file: ' . $destination);
            }
    ...
    protected function copyDirectoryRecursive(string $src, string $dst): void
    {
        if (!is_dir($dst) && !@mkdir($dst, 0775, true) && !is_dir($dst)) {
            throw new RuntimeException('Cannot create update directory: ' . $dst);
        }

        $items = scandir($src);
        if ($items === false) throw new RuntimeException('Cannot read directory: ' . $src);
        ...
        foreach ($items as $item) {
            ...
            if (is_dir($srcPath)) {
                ...
                $this->copyFileVerified($srcPath, $dstPath);
            }
        }
    }

    protected function cleanupDir(string $dir, bool $strict = false): void
    {
        if (!is_dir($dir)) return;

        $items = scandir($dir);
        if ($items === false) {
            if ($strict) throw new RuntimeException('Cannot read rollback directory: ' . $dir);
            return;
        }
        ...
        foreach ($items as $item) {
            ...
            $p = $dir . '/' . $item;
            if (is_dir($p)) {
                $this->cleanupDir($p, $strict);
            } else {
                if (!@unlink($p) && $strict) throw new RuntimeException('Cannot remove rollback file: ' . $p);
            }
        }
        if (!@rmdir($dir) && $strict) throw new RuntimeException('Cannot remove rollback directory: ' . $dir);
    }
