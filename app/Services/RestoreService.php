...
class RestoreService
{
    ...
    public function inspectBackup(string $zipPath): array
    {
        ...
        try {
            ...
            // Verify before any target database is connected or changed. Older backups
            // without a checksum remain supported, with an explicit unverified status.
            $checksumStatus = 'unverified';
            if (array_key_exists('sql_checksum', $manifest)) {
                $expected = $manifest['sql_checksum'];
                if (!is_string($expected) || !preg_match('/^[a-f0-9]{64}$/iD', $expected)) {
                    throw new RuntimeException('Invalid database checksum in backup manifest.');
                }
                $stream = $zip->getStream('database.sql');
                if ($stream === false) {
                    throw new RuntimeException('Could not read database dump for checksum verification.');
                }
                try {
                    $hash = hash_init('sha256');
                    if (hash_update_stream($hash, $stream) === false) {
                        throw new RuntimeException('Could not verify database dump checksum.');
                    }
                ...
                if (!hash_equals(strtolower($expected), $actual)) {
                    throw new RuntimeException('Backup database checksum mismatch. Restore was not started.');
                }
            ...
            return [
                ...
                'warnings'          => $checksumStatus === 'unverified' ? ['Legacy backup: database checksum is unavailable.'] : [],
    ...
    public function restoreBackup(
        ...
        try {
            ...
            if (!$zip->extractTo($stageDir, ['database.sql', 'manifest.json'])) {
                throw new RuntimeException('Could not extract backup database.');
            }
            ...
            // Recheck the exact extracted bytes immediately before executing them.
            if (isset($manifest['sql_checksum']) && !hash_equals(strtolower($manifest['sql_checksum']), (string)hash_file('sha256', $sqlFile))) {
                throw new RuntimeException('Extracted database checksum mismatch. Restore was not started.');
            }
